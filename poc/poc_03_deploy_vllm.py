...
Usage:
    uv run poc/poc_03_deploy_vllm.py

    # Use a different model
    uv run poc/poc_03_deploy_vllm.py --model "Qwen/Qwen2.5-7B-Instruct"

    # Use a specific GPU type
    uv run poc/poc_03_deploy_vllm.py --gpu-type "NVIDIA GeForce RTX 3090"

    # Reuse an already-running pod with vLLM (skips creation and cleanup)
...
def wait_for_vllm_health(base_url: str) -> None:
    ...
    while True:
        ...
        if elapsed > HEALTH_TIMEOUT_SECONDS:
            raise TimeoutError(
                f"vLLM health endpoint did not respond within {HEALTH_TIMEOUT_SECONDS}s"
            )

        try:
            response = httpx.get(health_url, timeout=10, follow_redirects=True)
            if response.status_code == 200:
                print(f"  [{elapsed:.0f}s] vLLM server is healthy!")
                return
...
def build_vllm_args(model: str) -> str:
    ...
    """
    return f"{model} --host 0.0.0.0 --port {VLLM_PORT}"


def step_create_pod(gpu_type: str, model: str) -> dict:
    """Create a pod running a vLLM server."""
    vllm_args = build_vllm_args(model)
    ...
    print(f"  Entrypoint: vllm serve {vllm_args}")
    print()
...
def step_wait_for_vllm(pod_id: str) -> str:
    """Wait for vLLM to become healthy and return the base URL."""
    print_section("Step 3: Wait for vLLM Health")
    base_url = get_proxy_url(pod_id, VLLM_PORT)
    wait_for_vllm_health(base_url)
    return base_url
...
def main():
    parser = argparse.ArgumentParser(
        description="POC 3: Deploy vLLM on RunPod"
    )
    parser.add_argument(
        "--gpu-type",
    ...
    print_section("POC 3: Deploy vLLM on RunPod")
    print("  Validates that vLLM boots on RunPod, loads a model, and")
    ...
    try:
        ...
        # Step 3: Wait for vLLM to be healthy
        base_url = step_wait_for_vllm(pod_id)
